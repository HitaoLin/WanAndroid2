 ...
 * </pre>
 */
public class RvWechartArticleAdapter2 extends RecyclerView.Adapter<RvWechartArticleAdapter2.ViewHolder> {

    private List<WechartArticleDatasBean> wechartArticleDatasBeanList;
    int pos;
...
//    }
    ...
    public RvWechartArticleAdapter2(List<WechartArticleDatasBean> wechartArticleDatasBeanList) {
        this.wechartArticleDatasBeanList = wechartArticleDatasBeanList;
    }
    ...
    public RvWechartArticleAdapter2.OnItemClickListener mOnItemClickListener;//第二步：声明自定义的接口

    //第三步：定义方法并暴露给外面的调用者
    public void setOnItemClickListener(RvWechartArticleAdapter2.OnItemClickListener listener) {
...
//        pos = getRealPosition(viewHolder);
        WechartArticleDatasBean articleDatasBean = wechartArticleDatasBeanList.get(i);
    ...
    public List<WechartArticleDatasBean> getDatas() {
        return wechartArticleDatasBeanList;
    }


    public void clearData() {
        wechartArticleDatasBeanList.clear();
        notifyItemRangeRemoved(0, wechartArticleDatasBeanList.size());

    }

    public void addData(List<WechartArticleDatasBean> datas) {
        addData(0, datas);

    }

    public void addData(int position, List<WechartArticleDatasBean> datas) {
        if (datas != null && datas.size() > 0) {
            wechartArticleDatasBeanList.addAll(datas);
            notifyItemRangeChanged(0, wechartArticleDatasBeanList.size());
        }
    }
...
//        return mHeaderView == null ? wechartArticleDatasBeanList.size() : wechartArticleDatasBeanList.size() + 1;
        return wechartArticleDatasBeanList.size();
    }
